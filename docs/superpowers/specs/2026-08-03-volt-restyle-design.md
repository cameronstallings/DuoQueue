...
- **Chat**: own bubble `#202817` solid; theirs `#14170F` + seam. No gradient fills. Timestamps/status in `type.tick` (voltDim for own "SENT/SENDING", textMuted for theirs). Spring-in entrance + haptics unchanged.
- **Match moment**: replaces merging aura discs — both avatars slide in, volt brackets `[ ]` close around the pair, single volt flash, "DUO LOCKED" in mono, letterspaced. Same timings/haptics as current moment.
